...
function getSettings() {
  try { return JSON.parse(localStorage.getItem('ml_settings') || '{}'); }
  catch { return {}; }
}
...
// ============================================
// BALANCE VISIBILITY (per-item eye toggle)
// ============================================

function getBalanceVisibility() {
  try {
    return JSON.parse(localStorage.getItem('ml_balance_vis') || '{}');
  } catch { return {}; }
}

function setBalanceVisibility(key, visible) {
...
function maskedOrReal(amount, key) {
  if (getSetting('hideBalances', false)) return '••••••';
  if (!isBalanceVisible(key)) return '••••••';
...
function getCategoryUsage() {
  try {
    return JSON.parse(localStorage.getItem('ml_cat_usage') || '{}');
  } catch { return {}; }
}
...
function recordVendorUsage(vendorLabel) {
  const vendors = getVendorPresets();
  const vendor  = vendors.find(v =>
    v.label.toLowerCase() === vendorLabel.toLowerCase()
  );
  if (vendor) {
...
function addVendorPreset(label) {
  const vendors = getVendorPresets();
  const exists  = vendors.find(v =>
...
function isAccountExcluded(accountId) {
  return getExcludedAccountIds().includes(accountId);
}
